...
// (services/webhook-engine/migrations) — no in-repo kind migration creates them — so,
...
// re-running boot is a no-op (idempotent). The services/webhook-engine tree is
// COPYd into the image under /repo by deploy/kind/control-plane/Dockerfile.
import { readFile } from 'node:fs/promises';
import { resolve } from 'node:path';

export const WEBHOOK_MIGRATIONS = [
  'services/webhook-engine/migrations/001-webhook-subscriptions.sql',
