...
export async function main(params: ActionParams): Promise<ActionResponse> {
  ...
  // Defensive belt-and-suspenders (tenant-query path only): if the caller is not a platform
  // caller, drop any cross-tenant shared row that may have leaked through (e.g. own-tenant
  // shared rows are allowed but rows owned by a different tenant must not appear).
