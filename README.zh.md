...
```text
   ...
   │ OpenBao（密钥）· Keycloak（按租户 realm 的 IAM + 面向 MCP 的 OAuth 2.1）· │
...
Redpanda（Kafka）、FerretDB + DocumentDB（兼容 MongoDB wire 协议的文档存储）、SeaweedFS（S3）与 OpenBao——外加一个 APISIX 网关与一个 action runner。
这是在本机获得可用环境最快的方式。
...
辅助脚本会为你配置健康检查、数据库迁移、FerretDB + DocumentDB 文档存储、SeaweedFS 存储桶与 OpenBao 审计设备：
...
| OpenBao（dev） | <http://localhost:58200> | 令牌 `root` |
...
| OpenBao 2.3.1 | 密钥管理 | `MPL-2.0` | [LICENSE](https://github.com/openbao/openbao/blob/main/LICENSE) |
...
>   SSPL/AGPL 风险敞口。
> - 将 Redpanda 作为 **Falcone 通过网络与之通信的独立后端服务** 来运行，本身并不会把它的许可证强加到
>   Falcone 的 MIT 代码上（不存在链接 / 衍生作品）。**但是**，它的 “以服务形式提供” / “竞争性服务”
>   条款，对一个**再次对租户暴露**其功能的多租户 BaaS 是直接相关的——比如 Kafka/事件 API。特别是，
>   Redpanda 的 BSL 授权排除了竞争性的托管服务。在任何托管或商业化提供之前，请审阅这些条款。
>   若其条款不适合你的用例，Redpanda 可在部署层替换。
