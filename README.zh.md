...
```text
   ...
   （Saga、appliers）          scheduling-engine / backup-status   (pg & mongo → Kafka)
            ...
            ▼                           ▼                             ▼
   ┌────────────────────────────────────────────────────────────────────────┐
   │ PostgreSQL（RLS + 按租户 schema）· MongoDB · Kafka · SeaweedFS ·         │
...
- **对象存储 —— 从 MinIO 迁移到 SeaweedFS。** **SeaweedFS**（Apache-2.0）是既定的未来对象存储
  （[ADR-13](docs-site/architecture/adrs.md)）；它由 umbrella chart 部署，MinIO 仅在切换窗口期间保留。
  参见 [SeaweedFS 存储运行手册](docs-site/architecture/seaweedfs.md)。
- **文档数据库替代方案** —— *计划中、评估中。* 平台当前使用 **MongoDB**（文档 API）；正在评估一个
  source-available 替代方案（在 DocumentDB 兼容后端之上的 FerretDB），并可在部署层替换。
...
| **FerretDB + DocumentDB** | 按租户/工作区的文档数据 API、管理、变更流（change streams）、指标与审计。 |
...
Redpanda（Kafka）、MongoDB（单节点副本集）、SeaweedFS（S3）与 Vault——外加一个 APISIX 网关与一个 action runner。
这是在本机获得可用环境最快的方式。
...
辅助脚本会为你配置健康检查、数据库迁移、Mongo 副本集、SeaweedFS 存储桶与 Vault 审计设备：
...
| MongoDB Server 7 | 按租户/工作区的文档数据 API | ⚠ `SSPL-1.0` | [mongodb.com](https://www.mongodb.com/legal/licensing/community-edition) |
...
| SeaweedFS 4.33 | 兼容 S3 的对象存储（未来方向，[ADR-13](docs-site/architecture/adrs.md)） | `Apache-2.0` | [seaweedfs](https://github.com/seaweedfs/seaweedfs) |
| MinIO | 兼容 S3 的对象存储（遗留 —— 切换期间保留） | ⚠ `AGPL-3.0` | [LICENSE](https://github.com/minio/minio/blob/master/LICENSE) |
...
| Apache OpenWhisk | 旧的 / 可选的函数引擎 | `Apache-2.0` | [openwhisk](https://github.com/apache/openwhisk) |
...
| MongoDB Node Driver（`mongodb`） | MongoDB 客户端 | `Apache-2.0` | [node-mongodb-native](https://github.com/mongodb/node-mongodb-native) |
...
> - **MongoDB（`SSPL-1.0`）**、**MinIO（`AGPL-3.0`）**、**Redpanda（`BSL-1.1` + `RCL`）** 与
>   **Vault（`BUSL-1.1`）** 属于 copyleft 或源代码可得（source-available）。
> - 将它们作为 **Falcone 通过网络与之通信的独立后端服务** 来运行，本身并不会把它们的许可证强加到
>   Falcone 的 MIT 代码上（不存在链接 / 衍生作品）。**但是**，它们的 “以服务形式提供” / “竞争性服务”
>   条款，对一个**再次对租户暴露**其功能的多租户 BaaS 是直接相关的——比如 Mongo 数据 API、Kafka/事件
>   API、S3 存储 API。特别是，**SSPL 第 13 条与 AGPL 第 13 条针对的正是以服务形式提供该软件的功能**，
>   而 Redpanda/Vault 的 BSL 授权排除了竞争性的托管服务。在任何托管或商业化提供之前，请审阅这些条款。
>   若其条款不适合你的用例，这四者都可在部署层替换。
> - **对象存储：MinIO → SeaweedFS（Apache-2.0）。** 依据
>   [ADR-13](docs-site/architecture/adrs.md)，**SeaweedFS** 是既定的未来对象存储，专门为消除 MinIO 的
>   **AGPL 第 13 条** “以服务形式提供” 风险而选定——面向一个再次对租户暴露 S3 的 BaaS。MinIO 仅在切换窗口
>   期间保留。
