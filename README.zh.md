...
- **MCP 服务器托管** *(开发中)*——租户将能够把自己的后端（数据、存储、函数）以
  [Model Context Protocol](https://modelcontextprotocol.io) 服务器的形式对外暴露，使任何兼容 MCP 的
  代理都能在该租户自身的隔离、认证与配额之下发现并调用它。
- **代理式工作流（Agentic workflows）**——基于 Temporal 的 **Flows** 引擎让租户可以用 JSON Schema
...
- **MCP 服务器托管** —— *积极开发中。* 让租户能够暴露并托管
  [Model Context Protocol](https://modelcontextprotocol.io) 服务器，使其后端可被 AI 代理访问，并处于
  按租户的隔离、认证与配额之下。
- **Flows —— 持久化工作流引擎（Temporal）** —— *进行中*（[epic #355](https://github.com/gntik-ai/falcone/issues/355)）。
  由租户通过 JSON Schema DSL 与解释器 worker 定义的工作流，凭据按租户隔离的第一方活动目录，触发器
  （Temporal Schedules、Webhook、平台事件）以及 Web 控制台中的可视化设计器。
...
| **Flows（工作流引擎）** | 由租户在基于 Temporal 的引擎上定义的持久化工作流：JSON Schema DSL 与解释器 worker、凭据按租户隔离的第一方活动目录、触发器（schedules、Webhook、平台事件），以及控制台中的可视化设计器。*积极开发中（[epic #355](https://github.com/gntik-ai/falcone/issues/355)）。* |
...
| MinIO | 兼容 S3 的对象存储 | ⚠ `AGPL-3.0` | [LICENSE](https://github.com/minio/minio/blob/master/LICENSE) |
...
| AWS SDK for JS v3（`@aws-sdk/client-s3`） | S3 / MinIO 客户端 | `Apache-2.0` | [aws-sdk-js-v3](https://github.com/aws/aws-sdk-js-v3) |
