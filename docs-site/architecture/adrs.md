...
**Evidence.** `docs/reference/postgresql/tenant-isolation-baseline.sql`; executor `SET LOCAL ROLE`; `packages/adapters/src/*`.
...
**Evidence.** `apps/control-plane-executor/src/runtime/server.mjs::resolveIdentity`.
...
**Evidence.** `deploy/gateway-config/public-route-catalog.json`; scope-enforcement plugin + specs.
...
**Evidence.** `packages/adapters/src/*` (`build*Plan`) → `apps/control-plane-executor/src/runtime/*-executor.mjs`.
...
**Decision.** Deliver realtime via **Server-Sent Events** (no WebSocket dependency), sourced from the document store's Postgres logical-replication CDC (`pgoutput` on the DocumentDB engine; see [ADR-14](/architecture/adrs#adr-14-migrate-document-store-from-mongodb-to-ferretdb-v2-documentdb)) and PostgreSQL trigger CDC, with tenant matching done **inside** the pipeline/channel.
...
**Evidence.** `packages/internal-contracts/src/domain-model.json` (`lifecycle_transitions`).
...
- *Spec baseline.* Keep the hosted runtime stateless, persist control-plane MCP state in PostgreSQL, pin contracts to the **2025-11-25** stable spec, and treat **2026-07-28** as an announced RC — verify wording before locking any server contract (notably the Tasks extension used by workflows-as-tools).
...
## ADR-14 — Migrate document store from MongoDB to FerretDB v2 + DocumentDB

**Findings.** Version-pinned compatibility matrix (aggregation, indexes, transactions, change streams), the per-tenant tenancy spike, the colocated-vs-dedicated decision, and gap remediation paths with downstream owners — all grounded in a live instance (reproducible harness + raw captures recorded with the spike findings).

**Runbook.** Operational architecture, two-layer topology, pinned image pair + upgrade order, PostgreSQL extension prerequisites, the verified tenancy model, the known-compatibility-differences table, change-stream remediation, observability and licensing: [FerretDB + DocumentDB Document-Store Runbook](/architecture/ferretdb).

**Decision.** Replace the bundled **MongoDB** document store with **FerretDB v2** — Apache-2.0, Linux Foundation governance — which speaks the MongoDB wire protocol over a **PostgreSQL 17 + DocumentDB** storage engine. Pin the migration to the version pair **`ghcr.io/ferretdb/ferretdb:2.7.0`** (`sha256:5706414241eb84f0515512c37b46db0f1b1eac9e5ceb7e4c2523211c184b1985`) / **`ghcr.io/ferretdb/postgres-documentdb:17-0.107.0-ferretdb-2.7.0`** (`sha256:2386795ec2aa7ae559304361979f1dc5708d383ee9020ae63dadc2940dfe58f7`). Run the document store on a **dedicated** Postgres instance (the `postgres-documentdb` image), **not** colocated on Falcone's existing relational Postgres, because `pg_documentdb_core`/`pg_documentdb` ship only in that image and require `shared_preload_libraries` (a server-start setting, not session-settable) — colocation would force replacing the relational `bitnami/postgresql` image for every tenant's RLS schemas. **Reject** Percona Server (SSPL), native-JSONB (not MongoDB-wire-compatible), ArangoDB (BSL), RavenDB (AGPL), and Couchbase (source-available).

**Why.**
- *Licence misfit.* MongoDB's **SSPL** is a source-available copyleft licence incompatible with an open-source BaaS that exposes the document store's wire protocol directly to tenants. FerretDB is **Apache-2.0** under **Linux Foundation** governance, removing the licensing risk while preserving the MongoDB driver/wire contract Falcone's clients already use.
- *Wire compatibility validated against Falcone's actual usage (live).* `hello` reports `maxWireVersion:21` (MongoDB 7.0 wire level), `buildInfo.version 7.0.77`, `ferretdb v2.7.0`. All **15** aggregation stages allowed by `packages/adapters/src/mongodb-data-api.mjs` (`$match`/`$project`/`$sort`/`$limit`/`$skip`/`$group`/`$unwind`/`$lookup`/`$count`/`$facet`/`$addFields`/`$set`/`$unset`/`$replaceRoot`/`$replaceWith`) are **SUPPORTED**; `$group` `$sum`/`$avg` over mixed int+double is exact; all five index types (single, compound, unique→`E11000`, sparse, TTL→purged in ~45s) are **SUPPORTED**. No PARTIAL aggregation/index entries.
- *Postgres leverage + AI-ready path.* The engine is PostgreSQL 17.6 with the document data stored as rows in `documentdb_data`, so Falcone reuses its Postgres operational tooling. The engine image **bundles `vector 0.8.1` (pgvector)** alongside `postgis 3.6.0` and `rum 1.3` — an in-place vector-search path for tenant documents — and Falcone's schema-per-tenant **RLS** (non-BYPASSRLS `falcone_app` role + `app.tenant_id` GUC) **coexists cleanly** with the DocumentDB extensions on the same instance (verified: RLS still enforces, no DDL conflict).

**Evidence.** The pinned image **tags and digests** for both `ferretdb` and `postgres-documentdb` are recorded above. **Upgrade order: start the DocumentDB engine first, then the FerretDB gateway** — the `pg_documentdb` libraries are preloaded via `shared_preload_libraries` and the `documentdb_api` schema must exist before the gateway's first wire handshake, so gateway-first fails. Engine GUCs observed: `shared_preload_libraries=pg_cron,pg_documentdb_core,pg_documentdb`, `cron.database_name=postgres`, `wal_level=logical` (the enabling GUC for the logical-replication realtime path). The full per-feature matrix (aggregation task 2, indexes task 3, transaction gap task 4, change-stream gap task 5, tenancy spike task 7, colocated-vs-dedicated task 8), its remediation paths, the reproducible harness (`compose.yaml`, probes `probe-main.mjs`/`probe-refine.mjs`/`probe-tenancy.mjs`/`probe-ttl.mjs`) and raw captures are in the findings file linked above; code touchpoints `packages/adapters/src/mongodb-data-api.mjs` (`applyTenantScopeToFilter`/`injectTenantIntoDocument`, `AGGREGATION_BLOCKED_STAGES`), `apps/control-plane-executor/src/runtime/mongo-data-executor.mjs`, `apps/control-plane-executor/src/runtime/realtime-executor.mjs` (#454).

**Risks / caveats.**
- *Change streams are UNSUPPORTED — highest severity.* `collection.watch()` returns `CommandNotSupported (115)` (`$changeStream is not supported yet`) and `collMod changeStreamPreAndPostImages` returns `UnknownBsonField (40415)`. This blocks **both** `apps/control-plane-executor/src/runtime/realtime-executor.mjs` and `packages/mongo-cdc-bridge/src/ChangeStreamWatcher.mjs`. Remediation is **re-architect** via a Postgres **logical replication slot + `pgoutput`** on the `documentdb_data` tables, with `REPLICA IDENTITY FULL` substituting for pre-images (needed to tenant-scope deletes); no native-wire shim exists at this version pair — escalated to the realtime/CDC children.
- *Multi-document transactions are UNSUPPORTED.* `commitTransaction` returns `CommandNotFound (59)` and the in-flight write **persists with no atomicity**; `abortTransaction` is a **silent no-op that does not roll back**. The `mongodb-data-api.mjs` transaction op (declared `snapshot`/`majority`) must be **shimmed to single-operation semantics** (DocumentDB guarantees single-doc atomicity) and reject true multi-doc atomic requests at the API boundary, or be dropped.
- *Per-database role isolation is NOT a boundary.* A user scoped `readWrite` on one tenant database successfully read another tenant's database; DocumentDB 0.107 does not enforce per-database role scoping. Falcone's **application-layer `tenantId` scoping** (`applyTenantScopeToFilter`/`injectTenantIntoDocument`) remains the **authoritative** isolation mechanism; hard DB-level isolation requires a dedicated DocumentDB database/instance per tenant tier.
- *Aggregation policy nuance.* `$out`, `$merge`, and `$geoNear` are actually **accepted** by the engine (not rejected as assumed); Falcone keeps blocking them via the adapter allowlist (`AGGREGATION_BLOCKED_STAGES`) — no engine-side guarantee to rely on.
- *Version divergence.* Every downstream child is coded to **2.7.0 / 0.107**; any upgrade must re-run the affected matrix cells before adoption.
- *Colocated-Postgres extension coupling.* `shared_preload_libraries` cannot be patched per-session, which is exactly why the **dedicated** instance is chosen; revisiting colocation would re-introduce a whole-instance restart/image change.

**Rejected alternatives.**
- *Percona Server for MongoDB* — distributed under **SSPL**, the same source-available copyleft that makes stock MongoDB a misfit; swapping vendors does not change the licence problem.
- *Native PostgreSQL JSONB* — would drop the **MongoDB wire protocol**, breaking every existing tenant MongoDB driver and the `mongodb-data-api.mjs` contract; a client-rewrite, not a migration.
- *ArangoDB* — core is **BSL** (Business Source License), a non-open delayed-source licence that fails the open-source BaaS bar.
- *RavenDB* — **AGPLv3** network-copyleft, a legal misfit for a document store exposed directly to tenants (same class of problem as MinIO's AGPL in ADR-13).
- *Couchbase* — **source-available** (BSL-style) rather than OSI open-source, and not MongoDB-wire-compatible, so it offers neither the licence nor the drop-in-driver property required.
