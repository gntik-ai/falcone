...
**Findings.** Version-pinned compatibility matrix (aggregation, indexes, transactions, change streams), the per-tenant tenancy spike, the colocated-vs-dedicated decision, and gap remediation paths with downstream owners — all grounded in a live instance: `openspec/changes/archive/2026-06-14-add-ferretdb-adr-spike/spike-findings.md` (reproducible harness + raw results under that change's `evidence/`).
...
**Evidence.** Spike: `openspec/changes/archive/2026-06-14-add-ferretdb-adr-spike/spike-findings.md` — the version-pinned compatibility matrix (aggregation/indexes/transactions/change-streams SUPPORTED/UNSUPPORTED + remediation paths), the tenancy spike, and the colocated-vs-dedicated decision, all grounded in a live `2.7.0` / `17-0.107.0-ferretdb-2.7.0` instance. Reproducible harness + raw captures under that change's `evidence/`: `compose.yaml` (the spike's FerretDB-over-DocumentDB stack), the probes `probe-main.mjs` / `probe-refine.mjs` / `probe-tenancy.mjs` / `probe-ttl.mjs`, and their results `result-main.json` / `result-refine.json` / `result-tenancy.json` / `result-ttl.json` (wire-version/aggregation/index matrix, refined aggregation + cross-database `$lookup`, the per-database isolation-gap probe, and TTL purge timing). Engine GUCs observed: `shared_preload_libraries=pg_cron,pg_documentdb_core,pg_documentdb`, `cron.database_name=postgres`, `wal_level=logical`. Code touchpoints: `services/adapters/src/mongodb-data-api.mjs` (`applyTenantScopeToFilter`/`injectTenantIntoDocument`, `AGGREGATION_BLOCKED_STAGES`), `apps/control-plane/src/runtime/mongo-data-executor.mjs`, `apps/control-plane/src/runtime/realtime-executor.mjs`, `services/mongo-cdc-bridge/src/ChangeStreamWatcher.mjs`. OpenSpec change: `add-ferretdb-adr-spike` (#454).
