...
const DNS_SUBDOMAIN_PATTERN = /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/

function isAllowedInternalHttpHostname(hostname: string, { allowBareInternalHttp = false } = {}): boolean {
...
export function normalizeServiceBaseUrl(rawValue: string, label: string, { allowBareInternalHttp = false } = {}): string {
  ...
  if (parsed.protocol === 'http:' && !isAllowedInternalHttpHostname(parsed.hostname, { allowBareInternalHttp })) {
...
export function buildServiceUrl(baseUrl: string, relativePath = ''): string {
...
export function assertDnsSubdomain(value: string, label: string): string {
  const normalized = String(value ?? '').trim().toLowerCase()
  if (!DNS_SUBDOMAIN_PATTERN.test(normalized)) {
    throw new Error(`${label} must be a valid DNS subdomain`)
  }
  return normalized
}
