...
async function emitKafkaEvent(payload) {
    if (!KAFKA_BROKERS) {
...
function isSimulationOperation(operation) {
    return operation.metadata?.execution_mode === 'simulation';
}
export async function dispatch(operationId) {
    ...
    // Audit: started
    void emitAuditEvent({
        eventType: isSimulationOperation(operation) ? 'restore.simulation.started' : `${operation.type}.started`,
    ...
    if (isSimulationOperation(operation)) {
        try {
            const result = await runRestoreSimulation({
                operation,
                deploymentProfile: operation.metadata?.target_environment
    ...
    try {
        ...
        if (operation.type === 'backup') {
            result = await withTimeout(actionAdapter.triggerBackup(operation.instanceId, operation.tenantId, context), timeoutS * 1000);
        }
        else {
            result = await withTimeout(actionAdapter.triggerRestore(operation.instanceId, operation.tenantId, operation.snapshotId, context), timeoutS * 1000);
        }
        ...
        // Audit: completed
        void emitAuditEvent({
            eventType: `${operation.type}.completed`,
            operationId: operation.id,
            tenantId: operation.tenantId,
            componentType: operation.componentType,
            instanceId: operation.instanceId,
        ...
        // Audit: failed
        void emitAuditEvent({
            eventType: `${operation.type}.failed`,
            operationId: operation.id,
            tenantId: operation.tenantId,
            componentType: operation.componentType,
            instanceId: operation.instanceId,
