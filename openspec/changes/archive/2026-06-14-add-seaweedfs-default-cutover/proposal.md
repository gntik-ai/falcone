...
- Migrate `tests/env/docker-compose.yml` from the single `minio` service (`minio/minio:latest`, host `:59000`/`:59001`) to an all-in-one SeaweedFS `server` (master / volume / filer / S3 gateway) with the S3 gateway on container `:8333` mapped to host `:58333`. The shared dev compose uses SeaweedFS's embedded filer store (see design D3); filer-on-PostgreSQL stays a chart/slice concern.
