...
- **ADR #356 tenancy decision**: multi-namespace vs. single-namespace with task-queue-per-tenant — the `TEMPORAL_TASK_QUEUE` and `TEMPORAL_NAMESPACE` env vars read from configuration. Defaults at apply time: `TEMPORAL_NAMESPACE=falcone-flows` (the shared flows namespace from ADR-11), `TEMPORAL_TASK_QUEUE=flows-main`. The real-stack tests override the namespace to `default` (the auto-setup dev image's pre-created namespace).
- **Expression engine selection**: RESOLVED at apply time — `cel-js@0.5.0` (CEL), per ADR-11 and the `expressionEngine` block in `flow-definition-mapping.json` (JSONata recorded as the validated fallback). cel-js evaluation runs in the `evaluateExpression` activity (outside the workflow sandbox).

## Apply-time additions (recorded during implementation)

- **D7 — Component-wrapper probe rendering**: the shared `charts/component-wrapper` workload template did not render container probes or `terminationGracePeriodSeconds`. They were added additively (gated by `with`) so the worker Deployment satisfies the spec's probe + graceful-shutdown requirement without affecting sibling components. The `app.kubernetes.io/component: flows-worker` NetworkPolicy label is injected via `podLabels`.
- **D8 — ISO-8601 duration normalisation**: the DSL uses ISO-8601 durations (FLW-E008) but the Temporal SDK's `Duration` / `compileRetryPolicy` reject ISO-8601 strings at runtime. `shared/mapping.ts#isoDurationToMs` (pure/deterministic) converts every DSL duration (wait, approval timeout, retry intervals, activity timeouts) to milliseconds before it reaches the SDK. Surfaced by the real-stack retry test.
- **D9 — Component-alias registry**: registering the `workflowWorker` alias required syncing `scripts/lib/deployment-chart.mjs`, `scripts/lib/deployment-topology.mjs`, `deployment-topology.json` (`packaging_guidance.component_aliases`), and the matching contract test — otherwise the chart-consistency validators in CI `quality` fail.
- **D10 — Activity-interface envelope**: the #360 plug-in seam is `ActivityInput` → `ActivityResult` in `services/workflow-worker/src/shared/types.ts`: `{ nodeId, taskType, node, params, tenant }`. The interpreter never invokes a task activity without a `TenantContext` (BaaS isolation rule). Node-ID naming convention: `activityId = node.id` (`shared/naming.ts#activityIdForNode`), asserted from history by the node-ID real-stack test.
