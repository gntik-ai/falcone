...
- **Executor unit tests run under `node --test`, not Vitest (task 7.1).** `flow-monitoring-executor.mjs` is a backend control-plane module; the repo convention is to unit-test such modules with `node --test` under `tests/unit/` (e.g. `tests/unit/flow-trigger-registry.test.mjs`), not Vitest (which is scoped to `apps/web-console`). The granular pure-function + tenant-isolation + replay + resume coverage lives in `tests/unit/flow-monitoring-executor.test.mjs`; the public HTTP/SSE surface is covered black-box in `tests/blackbox/flows-execution-events.test.mjs`.
- **Terminal-status normalisation.** The real Temporal SDK surfaces two casings for the same state — `describe().status.name` returns the protobuf enum form (`COMPLETED`), while `workflow.list()` returns the visibility form (`Completed`). The executor normalises (strip `WORKFLOW_EXECUTION_STATUS_` prefix, drop separators, lowercase) so the live-follow terminal check is robust across both; without this the real-stack stream never reaches `stream-end`. Surfaced by the `tests/env/flows-monitoring/` real-stack proof.
- **SSE handler is a dedicated `runFlowMonitoringSse` (not `runRealtimeSse` verbatim).** Task 2.2 referenced reusing `runRealtimeSse`. The monitoring stream needs two things the realtime handler does not emit: a per-frame `id:` line (for `Last-Event-ID` resume) and a terminal `event: stream-end` frame that closes the stream. `runFlowMonitoringSse` reuses the same SSE preamble/ping/cleanup idiom as `runRealtimeSse` but adds those two affordances, and propagates a pre-stream tenant rejection (403) as a hard HTTP status rather than a 200 error frame.
- **Monitoring executor injection gate.** Wired WITH the flow executor (it needs a Temporal client) on the same `TEMPORAL_ADDRESS` guard; `FLOWS_ENABLED=false` is the explicit off-switch (the SSE route then falls through to the 501 guard). The history provider lazily connects its own Temporal client, mirroring the trigger registry's lazy-connect boundary.
- **Run-view live stream is opt-in via an anon-key input.** Mirroring the existing realtime console (`RealtimeConsole.tsx`), the run view takes the anon key from a user-supplied input to open the live `EventSource`; a terminal run renders statically from the persisted detail endpoint with no stream opened.
