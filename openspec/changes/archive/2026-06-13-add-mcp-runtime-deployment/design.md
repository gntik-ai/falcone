...
- **Provisioning MCP domain mirrors workflows (teardown cascade).** The reuse-Knative model has no separate provision-time footprint apply: the chart ships the static RBAC/NetworkPolicy, and individual MCP servers are created later via the MCP API (#391/#392/#394). So the saga integration is a **teardown** in `tenant-purge-sweep.mjs`'s `TEARDOWN_PLAN`, exactly like `workflows-applier.mjs` — delete the tenant's MCP-server ksvcs + MCP metadata rows, dependency-injected, idempotent, `42P01`→skipped, errors→partial-failure (purge not finalized). No new orchestration pattern. *Alternative considered:* a provision-time `apply` like the OpenWhisk `functions-applier` — rejected because MCP servers are not reprovisioned config; they are created on demand via the API.
