...
- OQ2: Should the IAM client use the SeaweedFS admin root credential or a dedicated admin service account? **RESOLVED:** SeaweedFS 4.33 exposes no signed HTTP identity API — the real admin path is `weed shell s3.configure -apply` (verified against the pinned image; the deployment chart seeds a bootstrap admin via the static `-s3.config` secret). The IAM client therefore uses `createWeedShellTransport({ exec })`, where `exec` runs `weed shell` via `kubectl exec`/`docker exec` with the deployment's admin credentials; the static/bootstrap admin identity is never rewritten or deleted. A signed-HTTP transport remains available for any future admin-API surface, but the `weed shell` transport is the validated path (real-stack slice green on the kind cluster).
