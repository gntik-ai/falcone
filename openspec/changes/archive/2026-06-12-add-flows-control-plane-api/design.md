...
**D7-note (implementation deviation — verified against code):** `services/internal-contracts/src/public-route-catalog.json` is GENERATED from the unified OpenAPI document + taxonomy + gateway-routing YAML (`scripts/lib/public-api.mjs`); `validate:public-api` and `validate:gateway-policy` enforce that every catalog route maps to an OpenAPI operation and that every *routing family* has a Helm-chart APISIX route (`public-api-<id>`) plus `gatewayPolicy.familyPolicies.<id>`. Adding a real `flows` routing family would therefore require out-of-scope edits to the Helm chart (`charts/in-falcone/values*.yaml`) and `services/gateway-config/base/public-api-routing.yaml`. The established, in-scope convention (verified: the immediately-preceding `write-time-auto-embedding` change registered its public routes the same way) is the hand-maintained flat allow-list `services/gateway-config/public-route-catalog.json` carrying `{method, path, privilege_domain}`. The `gatewayRouteClass` split above maps 1:1 onto `privilege_domain`: `control → structural_admin`, `data-control → data_access`. The generated OpenAPI catalog + gateway-routing family wiring for flows is deferred to the gateway-integration sibling (e.g. #365); this change registers the flows surface in the flat allow-list and enforces the privilege-domain split with a black-box test.
...
- **OQ2** (RESOLVED): Per ADR-11 / #356 (`services/workflow-worker/src/worker.ts`), the search attributes `tenantId`, `workspaceId`, `flowId`, `flowVersion`, `triggerType` are all typed `Keyword`. The env-var convention is `TEMPORAL_ADDRESS` (default `127.0.0.1:7233`), `TEMPORAL_NAMESPACE` (default `falcone-flows`), `TEMPORAL_TASK_QUEUE` (default `flows-main`). The flow-executor uses exactly these names and `Keyword`-typed equality filters in the visibility query.
