...
- [ ] 1.1 `deploy/kind/control-plane/routes.mjs`: register one new route
...
- [ ] 1.2 `deploy/kind/control-plane/fn-handlers.mjs`: add `secretReplace(ctx)` — mirror `secretSet`
  ...
  (no value).
- [ ] 1.3 `deploy/kind/control-plane/fn-handlers.mjs`: make `secretSet` (`POST`) **create-only** — before
...
- [ ] 1.4 `deploy/kind/control-plane/fn-handlers.mjs`: widen `secretSet`, `secretList`, `secretGet`, and
...
- [ ] 1.5 `deploy/kind/control-plane/vault-secrets.mjs`: extend `createWorkspaceSecretStore` to
...
- [ ] 1.6 `resolvedRefCount`: compute best-effort from the workspace's function registry — scan the
...
- [ ] 1.7 Confirm `name`/`secretName` and `value`/`secretValue` aliasing on the write side stays
...
- [ ] 1.9 Verify `npm run generate:public-api` produces **no diff** — this confirms only that **no contract
...
- [ ] 2.1 New client `apps/web-console/src/services/secretsApi.ts` mirroring `functionsApi.ts`:
...
- [ ] 2.2 New page `apps/web-console/src/pages/ConsoleWorkspaceSecretsPage.tsx` (distinct from the mock
  ...
  empty state).
- [ ] 2.3 Write-only value handling in the UI: masked value input, never pre-populated from any read,
  cleared from component state after submit; no value in the DOM/URL/query/log/telemetry; no
  "reveal"/"show value" affordance.
- [ ] 2.4 New route in `apps/web-console/src/router.tsx` (e.g. `path: 'workspace-secrets'`) wrapped in a
...
- [ ] 2.5 New nav entry "Workspace Secrets" in `apps/web-console/src/layouts/ConsoleShellLayout.tsx`,
...
- [ ] 2.6 States: distinct loading, empty (naming the active workspace + environment), client-side
...
- [ ] 3.1 No contract change: confirm the five `function_workspace_secret` routes in
  ...
  after §1 convergence).
- [ ] 3.2 Console read types == OpenAPI `FunctionWorkspaceSecret` / `FunctionWorkspaceSecretCollection`;
...
- [ ] 4.1 Backend black-box (`tests/blackbox/…`): route+handler+catalog wiring for all five ops incl. the
...
- [ ] 4.2 **Write-only assertion:** assert that **no secret value appears in any response body** of any
...
- [ ] 4.3 **Isolation / IDOR denial:** cross-tenant — tenant B targeting tenant A's workspace id →
...
- [ ] 4.4 Console behavior: create clears the value input; list/detail render metadata only; delete shows
...
- [ ] 4.5 Run `bash tests/blackbox/run.sh` (secrets subset) green; run the console test build where
  applicable (respecting the known-broken web-console baseline — new tests pass, the failing set is
  unchanged).

## 5. Docs / nav

- [ ] 5.1 Document the Workspace Secrets console screen and the PUT-replace + metadata runtime convergence
...
- [ ] 5.2 Confirm the nav relabel cleanly disambiguates the new "Workspace Secrets" screen from the
