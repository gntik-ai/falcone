...
| 4 | data-services | DAT | PostgreSQL, MongoDB-compatible document store (FerretDB) and S3-compatible object storage (SeaweedFS) management plus tenant-scoped data CRUD, schemas, credentials, exports/imports | `/v1/postgres` (`postgres.openapi.json`), `/v1/mongo` (`mongo.openapi.json`), `/v1/storage` (`storage.openapi.json`) | ConsolePostgresPage, ConsoleMongoPage, ConsoleStoragePage | `services/adapters` |
| 5 | functions-runtime | FN | OpenWhisk-compatible functions API (Knative-backed runtime), action versions, triggers (cron / Kafka / storage / HTTP), packages, rules, function secrets, activation logs | `/v1/functions` (`functions.openapi.json`) | ConsoleFunctionsPage | `services/scheduling-engine` |
