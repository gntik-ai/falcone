...
`FunctionsConsole` use `Input`/`Select`/`Textarea`/`Label` for their text-entry fields. As
of #757, the row/document editors these pages render — `PostgresDataEditor` and
