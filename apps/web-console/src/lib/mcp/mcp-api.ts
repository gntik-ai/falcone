...
export async function fetchMcpServerDetail(serverId: string, signal?: AbortSignal): Promise<McpServerDetailInput> {
  return requestConsoleSessionJson<McpServerDetailInput>(`/v1/mcp/servers/${encodeURIComponent(serverId)}`, { signal })
}
...
export async function invokeMcpTool(
  ...
  return requestConsoleSessionJson<InvokeMcpToolResult>(
    `/v1/mcp/servers/${encodeURIComponent(serverId)}/playground/tool-calls`,
