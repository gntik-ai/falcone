...
function TenantStep({ data, context }: WizardStepProps<WorkspaceData>) { return <div><Label>Organización de contexto</Label><p className="mt-2 text-sm">{data.tenantId || context.tenantId || 'Sin organización activa'}</p></div> }
...
export function CreateWorkspaceWizard({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated?: () => void }) {
  ...
  return <WizardShell<WorkspaceData>
    ...
    description="Alta guiada de área de trabajo dentro de la organización activa."
    ...
    steps={[
      { id: 'tenant', label: 'Organización', component: TenantStep, validate: (data) => createValidation(!data.tenantId ? { tenantId: 'Selecciona una organización.' } : {}) },
    ...
    buildSummary={(data) => [{ label: 'Organización', value: data.tenantId ?? '' }, { label: 'Nombre', value: data.name ?? '' }, { label: 'Descripción', value: data.description ?? '' }, { label: 'Máx. funciones', value: data.maxFunctions ?? '' }, { label: 'Máx. bases de datos', value: data.maxDatabases ?? '' }]}
    onSubmit={async (data) => {
