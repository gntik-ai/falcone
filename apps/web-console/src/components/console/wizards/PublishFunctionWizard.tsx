...
import { parseRequiredIntegerField } from '@/lib/console-create-form-validation'
...
function RuntimeStep({ data, onChange, validation }: WizardStepProps<FnData>) { return <div className="grid gap-4 sm:grid-cols-3"><div className="space-y-2"><Label htmlFor="fn-runtime">Runtime</Label><Select id="fn-runtime" value={data.runtime ?? ''} onChange={(e) => onChange({ runtime: e.target.value })}><option value="">Selecciona runtime</option><option value="nodejs:18">nodejs:18</option><option value="nodejs:20">nodejs:20</option></Select>{validation.fieldErrors.runtime ? <p className="text-sm text-destructive">{validation.fieldErrors.runtime}</p> : null}</div><div className="space-y-2"><Label htmlFor="fn-memory">Memoria (MB)</Label><Input id="fn-memory" inputMode="numeric" aria-invalid={Boolean(validation.fieldErrors.memoryMb)} aria-describedby={validation.fieldErrors.memoryMb ? 'fn-memory-error' : undefined} className={validation.fieldErrors.memoryMb ? 'border-destructive' : undefined} value={data.memoryMb ?? '256'} onChange={(e) => onChange({ memoryMb: e.target.value })} />{validation.fieldErrors.memoryMb ? <p id="fn-memory-error" className="text-sm text-destructive">{validation.fieldErrors.memoryMb}</p> : null}</div><div className="space-y-2"><Label htmlFor="fn-timeout">Timeout (ms)</Label><Input id="fn-timeout" inputMode="numeric" aria-invalid={Boolean(validation.fieldErrors.timeoutMs)} aria-describedby={validation.fieldErrors.timeoutMs ? 'fn-timeout-error' : undefined} className={validation.fieldErrors.timeoutMs ? 'border-destructive' : undefined} value={data.timeoutMs ?? '30000'} onChange={(e) => onChange({ timeoutMs: e.target.value })} />{validation.fieldErrors.timeoutMs ? <p id="fn-timeout-error" className="text-sm text-destructive">{validation.fieldErrors.timeoutMs}</p> : null}</div></div> }
