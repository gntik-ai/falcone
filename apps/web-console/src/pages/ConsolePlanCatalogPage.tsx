...
export function ConsolePlanCatalogPage() {
  ...
  return <main className="space-y-6"><header className="rounded-3xl border border-border bg-card/70 p-6"><h1 className="text-2xl font-semibold">Plan catalog</h1><div className="mt-4 flex gap-3"><select aria-label="status-filter" value={state.status} onChange={(e) => { const status = e.currentTarget.value as api.PlanStatus | 'all'; setState((current) => ({ ...current, status, page: 1 })) }}><option value="all">All</option><option value="draft">Draft</option><option value="active">Active</option><option value="deprecated">Deprecated</option><option value="archived">Archived</option></select><Button type="button" onClick={() => navigate('/console/plans/new')}>Create Plan</Button></div></header><table className="w-full text-sm"><thead><tr><th>Slug</th><th>Name</th><th>Status</th><th>Assigned</th><th>Updated</th></tr></thead><tbody>{state.items.map((plan) => <tr key={plan.id} onClick={() => navigate(`/console/plans/${plan.id}`)}><td>{plan.slug}</td><td>{plan.displayName}</td><td><PlanStatusBadge status={plan.status} /></td><td>{plan.assignedTenantCount ?? 0}</td><td>{plan.updatedAt ?? '—'}</td></tr>)}</tbody></table></main>
}
