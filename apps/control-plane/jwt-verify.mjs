...
// (node:crypto + JWKS over fetch). Brings apps/control-plane/server.mjs to PARITY with the
// executor's apps/control-plane-executor/src/runtime/jwt-verify.mjs (fix-tenant-realm-token-issuance, #527).
//
// Falcone places each tenant in its OWN Keycloak realm whose NAME == the tenant id (see
// apps/control-plane/b-handlers.mjs: `realm = tenantId`). The control-plane previously
