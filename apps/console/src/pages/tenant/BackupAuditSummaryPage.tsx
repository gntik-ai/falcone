...
export function BackupAuditSummaryPage({ token, tenantId }: Props) {
  const [filters, setFilters] = useState<AuditQueryFilters>({ tenantId })
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Backup Activity History</h1>
      <AuditEventFilters
        role="tenant_owner"
        onChange={(patch) => setFilters((prev) => ({ ...prev, ...patch, tenantId }))}
      />
      {isLoading ? (
        <p>Loading...</p>
      ) : (
        <>
          <AuditEventTable events={events} role="tenant_owner" />
          {hasNextPage && (
            <button
