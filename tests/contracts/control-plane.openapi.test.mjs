...
test('control-plane OpenAPI document remains structurally valid', async () => {
  ...
  assert.ok(document.paths['/v1/functions/workspaces/{workspaceId}/actions']);
  assert.ok(document.paths['/v1/functions/actions/{resourceId}']);
  assert.ok(document.paths['/v1/functions/actions/{resourceId}/invocations']);
  assert.ok(document.paths['/v1/functions/actions/{resourceId}/activations']);
  assert.ok(document.paths['/v1/functions/actions/{resourceId}/activations/{activationId}']);
...
test('control-plane contract enforces versioning, authorization, family metadata, idempotent mutation expectations, and gateway hardening responses', async () => {
  ...
  const listFunctionActions = document.paths['/v1/functions/workspaces/{workspaceId}/actions'].get;
  const createFunctions = document.paths['/v1/functions/actions'].post;
  const getFunctions = document.paths['/v1/functions/actions/{resourceId}'].get;
  const updateFunctions = document.paths['/v1/functions/actions/{resourceId}'].patch;
  const deleteFunctions = document.paths['/v1/functions/actions/{resourceId}'].delete;
  const invokeFunctionAction = document.paths['/v1/functions/actions/{resourceId}/invocations'].post;
  const listFunctionActivations = document.paths['/v1/functions/actions/{resourceId}/activations'].get;
  const getFunctionActivation = document.paths['/v1/functions/actions/{resourceId}/activations/{activationId}'].get;
  ...
  const listFunctionActionsParameters = resolveParameters(document, listFunctionActions);
  const createFunctionsParameters = resolveParameters(document, createFunctions);
  const getFunctionsParameters = resolveParameters(document, getFunctions);
  const updateFunctionsParameters = resolveParameters(document, updateFunctions);
  const deleteFunctionsParameters = resolveParameters(document, deleteFunctions);
  const invokeFunctionActionParameters = resolveParameters(document, invokeFunctionAction);
  const listFunctionActivationsParameters = resolveParameters(document, listFunctionActivations);
  const getFunctionActivationParameters = resolveParameters(document, getFunctionActivation);
  ...
  assert.ok(document.components.schemas.FunctionAction.properties.source);
  assert.ok(document.components.schemas.FunctionAction.properties.execution);
  assert.ok(document.components.schemas.FunctionAction.properties.activationPolicy);
  assert.ok(document.components.schemas.FunctionAction.properties.httpExposure);
  assert.ok(document.components.schemas.FunctionAction.properties.storageTriggers);
  assert.ok(document.components.schemas.FunctionAction.properties.cronTriggers);
  assert.ok(document.components.schemas.FunctionActionCollection);
  assert.ok(document.components.schemas.FunctionDeploymentSource);
  assert.ok(document.components.schemas.FunctionExecutionConfiguration);
  assert.ok(document.components.schemas.FunctionExecutionLimits);
  assert.ok(document.components.schemas.FunctionActivationPolicy);
  assert.ok(document.components.schemas.FunctionHttpExposure);
  assert.ok(document.components.schemas.FunctionStorageTrigger);
  assert.ok(document.components.schemas.FunctionCronTrigger);
  assert.ok(document.components.schemas.FunctionInvocationWriteRequest);
  assert.ok(document.components.schemas.FunctionInvocationAccepted);
  assert.ok(document.components.schemas.FunctionActivation);
  assert.ok(document.components.schemas.FunctionActivationCollection);
  assert.ok(document.components.schemas.FunctionActivationLog);
  assert.ok(document.components.schemas.FunctionActivationResult);
  assert.ok(document.components.schemas.FunctionActivationRerunAccepted);
  assert.equal(document.components.schemas.FunctionQuotaStatus.required.includes('maxActionsPerWorkspace'), true);
  assert.equal(document.components.schemas.OpenWhiskProviderCompatibility.required.includes('supportedRuntimes'), true);
  assert.equal(document.components.schemas.FunctionServerlessNamingPolicy.required.includes('actionPrefix'), true);
  assert.equal(document.components.schemas.FunctionTenantIsolation.properties.actionPrefixIsolation.type, 'string');
  ...
  assert.equal(listFunctionActions['x-family'], 'functions');
  assert.equal(listFunctionActionsParameters.some((parameter) => parameter.name === 'workspaceId'), true);
  assert.equal(createFunctions['x-resource-type'], 'function_action');
  assert.equal(createFunctionsParameters.some((parameter) => parameter.name === 'Idempotency-Key'), true);
  assert.equal(getFunctions['x-resource-type'], 'function_action');
  assert.equal(getFunctionsParameters.some((parameter) => parameter.name === 'resourceId'), true);
  assert.equal(updateFunctionsParameters.some((parameter) => parameter.name === 'Idempotency-Key'), true);
  assert.equal(deleteFunctionsParameters.some((parameter) => parameter.name === 'Idempotency-Key'), true);
  assert.equal(invokeFunctionAction['x-resource-type'], 'function_invocation');
  assert.equal(invokeFunctionActionParameters.some((parameter) => parameter.name === 'Idempotency-Key'), true);
  assert.ok(invokeFunctionAction.responses['202']);
  assert.equal(listFunctionActivations['x-resource-type'], 'function_activation');
  assert.equal(listFunctionActivationsParameters.some((parameter) => parameter.name === 'resourceId'), true);
  assert.equal(getFunctionActivationParameters.some((parameter) => parameter.name === 'activationId'), true);
  assert.equal(getFunctionActivationLogs['x-resource-type'], 'function_activation_log');
  assert.equal(getFunctionActivationLogsParameters.some((parameter) => parameter.name === 'activationId'), true);
